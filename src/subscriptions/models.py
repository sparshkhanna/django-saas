...
class UserSubscription(models.Model):
    ...
    def billing_cycle_anchor(self):
        """
        https://docs/stripe.com/payments/checkout/billing_cycle
        Optional delay start to start new subscription in Stripe checkout
        """
        if not self.current_period_end:
            return None 
        return int(self.current_period_end.timestamp())
    def save(self, *args, **kwargs):
        if (self.original_period_start is None
